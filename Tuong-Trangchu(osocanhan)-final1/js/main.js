$(document).ready(function() {
	...
	$('.edit').each(function() {
		$(this).click(function() {
	       ...
	       if(selector == 'field'){
	       		...
	       		$('#myModalLabel').html('Chỉnh sửa chi tiết');
	       		$('#email').nextAll().removeClass('show');
	       		$('#email').prevAll().removeClass('show');
	       		$('#email').addClass('show');
	       }
    ...
    //hide chatbox
    $('._nameuserChat, ._NubFlyoutTitlebar ._icon5').click(function(){
    	var $offset = $('.boxChat').css('bottom');
    	if($offset == '0px') {
    		$('.boxChat').css('bottom', '-369px'); }
    	else {
    		$('.boxChat').css('bottom', '0px');
    	}
    });
    // active item message chat
    var $id = $('.Maincontentms').attr('id');
   	if($id) {
   		$('.mslistUser #'+ $id).css('background-color', '#F2F2F2');
   	}
   // search friends ms
  	$('.searchFms').click(function(){
  		$('.sidebarOption, .wpContent, ._iconmscall, ._mssttUser').hide();
  		$('.boxSearchFms, .infoUserMsNew, .itemMsNew').show();
  		$('.Maincontentms').css('width', '100%');
  		$('.mslistUser #'+ $id).css('background-color', '#fff');
  	});
   // close ms new
   $('.closeMsNew').click(function(){
   		$('.itemMsNew, .boxSearchFms, .infoUserMsNew').hide();
  		$('._iconmscall, .sidebarOption, .wpContent').show();
  		$('.Maincontentms').css('width', '66.66666667%');
  	});
   $('._rootPp_sticker').click(function(){
   		$('._popup_sticker').toggle();
   		$('._popup_smile').hide();
  	});
   $('._rootPp_smile').click(function(){
   		$('._popup_sticker').hide();
   		$('._popup_smile').toggle();
  	});
   ...
   // fix message click (i) and search list user chat
   $('._iconmscall ._iconinfo').click(function(){
   	if($(this).hasClass('_iconinfo_off')){
   		$(this).removeClass("_iconinfo_off");
   		$('.sidebarOption').show();
  		$('.Maincontentms').css('width', '66.66666667%');
   	}
   	else {
   		$(this).addClass("_iconinfo_off");
   		$('.sidebarOption').hide();
  		$('.Maincontentms').css('width', '100%');
   	}
   });
